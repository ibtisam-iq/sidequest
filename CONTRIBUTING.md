...
Create a file at `data/links/<category>/<slug>.yaml` or
...
| Category or country exists in `taxonomy/categories.yaml` | Stops typo'd categories creating orphan folders |
| Folder name matches the entry's `category`/`country` field | The two must agree or the site routes to the wrong place |
...
**Slugs are the filename.** `data/links/dev-tools/ghostty.yaml` has the slug `ghostty`. That's what
`alternatives: [ghostty]` refers to, and what appears in the URL.
...
**Adding one is expected, not exceptional.** You don't need permission:

- Via the **issue form**: just type the new category. It's registered automatically, and the PR
  says so.
- Via **`npm run add-link`**: choose *Create a new category* and give it a display name.
- **By hand**: append a record to the registry and create the matching folder.
