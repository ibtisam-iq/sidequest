...
- **URL** - the canonical page, not a redirect or a search result. `?utm_source=…` is fine, it
  gets stripped.
- **Category** - a top-level category, or `parent/sub` for one with subcategories (e.g.
  `dev-tools`, `ai-tools/ai-coding-agents`). Check
  [`taxonomy/categories.yaml`](taxonomy/categories.yaml) for the current tree and reuse a branch if
  it fits. A genuinely new category or subcategory is welcome (see below). Anything under
  `shadow-libraries/...` automatically gets a legal-risk warning badge - only use it for shadow
  libraries and similar copyright-risk resources, not as a shortcut for "unsure where this goes."
...
Create a file at `data/links/<parent>/<slug>.yaml` (or `data/links/<parent>/<sub>/<slug>.yaml` for
a subcategory) or `data/companies/<country>/<slug>.yaml`, then:
...
| Category path (or country) resolves in `taxonomy/categories.yaml` | Stops typo'd categories creating orphan folders. For links, both the parent and, if given, the subcategory must be registered under it |
...
| `legal_risk: true` is set on every entry under `shadow-libraries/...`, and never set elsewhere | A content-integrity rule, not a suggestion - it drives the visible warning badge |
...
**Slugs are the filename.** `data/links/dev-tools/cli-terminal/ghostty.yaml` has the slug
...
organized as a **two-level tree** for links - a top-level category, and optionally subcategories
underneath it (only two levels are supported; a subcategory never itself has children):
...
- slug: ai-coding-agents
  ...
  type: links
  parent: ai-tools  # makes this a subcategory of ai-tools
```

A link's `category` field is a path: `ai-tools` (flat) or `ai-tools/ai-coding-agents` (with a
...
**Adding a category or subcategory is expected, not exceptional.** You don't need permission:

- Via the **issue form**: type `parent` or `parent/sub`. Any level that doesn't exist yet is
  registered automatically, and the PR says so.
- Via **`npm run add-link`**: pick the top-level category first, then either an existing
  subcategory or *Create a new subcategory* - or *Create a new category* to add a whole new
  top-level branch.
- **By hand**: append a record to the registry (with `parent:` for a subcategory) and create the
  matching folder.

### The near-duplicate check

The thing that actually kills an open taxonomy is fragmentation - `ai-tool`, `ai-tools` and
`ai_tools` all existing because nobody checked. So a new category or subcategory is fuzzy-matched
(Levenshtein) against its **siblings only** - a new subcategory of `ai-tools` is checked against
the other subcategories of `ai-tools`, never against unrelated top-level categories or
subcategories under a different parent:
